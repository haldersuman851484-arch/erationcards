...
- Name: admin-order-list.spec.ts >> Admin order list >> updating order status from the detail dialog persists the new status in the order list
- Location: tests/admin-order-list.spec.ts:161:3
...
Locator: getByTestId('button-view-order-1')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByTestId('button-view-order-1')
