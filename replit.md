...
- **Public step 1 is panel-based (no family pop-up)** — numbered card panels ("Card 1" + green Primary badge for the main card), an optional per-card PDF dropzone that appears once that card's type/name/number are filled (20MB client cap mirroring the server), a "Do you want to print more cards?" prompt (No, Thanks / + Yes, Add More), and a "Your Cards • NN" summary with edit/delete (deleting re-keys held PDFs). Cards commit on Add More / Next; an untouched new panel is silently discarded. A held PDF is bound to the card's details at attach time (case/spacing cosmetic): if the type, name or number is edited afterwards the file is dropped with an "Attached PDF removed" toast — never let a file ride along to a different person's card (auto-upload double-checks this before sending). PDFs chosen at step 1 are held in the browser and auto-uploaded per card right after payment — if every card was covered and every upload succeeded the order submits itself, otherwise step 4 stays manual and "Retry upload" re-sends the held file. The operator flow keeps its dialog-based family-card editor.
...
2. **Set env vars** in hPanel → Node.js → Your App → Environment Variables (see `.env.example`):
   ...
   - `GA4_MEASUREMENT_ID` / `CLARITY_PROJECT_ID` — (optional) Google Analytics 4 + Microsoft Clarity. Injected server-side at serve time on public pages only (admin/operator/processing/receipt never tracked); adding or changing them needs only an app restart, no rebuild. Leave unset to disable. When GA4 is on, a `purchase` event (with order value in INR) fires every time a customer finishes an order — see it in GA4 under **Reports → Engagement → Conversions** (or Events). To count it as a conversion: GA4 → **Admin → Events**, find `purchase`, and toggle "Mark as key event/conversion" (GA4 usually marks `purchase` automatically). The order form also fires one event each time a customer finishes a step — `begin_checkout` (card details done), `add_shipping_info` (address done), `add_payment_info` (payment screenshot done), then `purchase` (order submitted). To see where customers drop off: GA4 → **Explore → Funnel exploration**, add those four events as steps in that order — the chart shows how many people reach each step and where they abandon.
...
- **Google Search Console (fully set up 2026-07-31)** — URL-prefix property `https://erationcards.in/` under the user's own Google account; ownership verified via HTML file `googleb39ebccdc08cdb68.html` in `artifacts/ration-card-portal/public/` — **never delete this file**; it ships in every build/Hostinger bundle (standard content `google-site-verification: <filename>`, recreatable from the name alone). sitemap.xml submitted; Request-indexing done for `/`, `/order`, `/guides/download-e-ration-card`; Googlebot live-crawled the homepage successfully the same night (prerender pipeline confirmed working for Google — correct title/description in crawled HTML). IndexNow ping of all sitemap URLs accepted. GSC quirks: a freshly submitted sitemap shows red "Couldn't fetch" until Google's first read (normal, self-heals — don't resubmit); page URLs accidentally submitted as sitemaps sit at "Couldn't fetch" harmlessly and are removed via each row's ⋮ menu (two stray entries `/` and `/guides/download-e-ration-card` may still be pending user cleanup). Brand-new domain: expect a few days–2 weeks to appear in Google; check progress in GSC → Indexing → Pages or by searching `site:erationcards.in`.
